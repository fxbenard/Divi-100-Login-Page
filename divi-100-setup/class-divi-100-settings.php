	...
	class Divi_100_Settings {
		...
		function default_field() {
			return array(
				...
				'preview_height'             => 182,
				'id'                         => 'field_id',
				'label'                      => __( 'Label', 'divi-100-login-page' ),
				'placeholder'                => '',
				'description'                => false,
				'options'                    => array(
					'value' => 'label',
				...
				'button_active_text'         => __( 'Change', 'divi-100-login-page' ),
				'button_inactive_text'       => __( 'Select', 'divi-100-login-page' ),
				'button_remove_text'         => __( 'Remove', 'divi-100-login-page' ),
				'media_uploader_title'       => __( 'Select Image', 'divi-100-login-page' ),
				'media_uploader_button_text' => __( 'Use This Image', 'divi-100-login-page' ),
				'default'                    => '#888888',
			);
		}
		...
		function render_settings() {
			...
			if ( isset( $_POST[ $nonce ] ) ) {
				...
				if ( wp_verify_nonce( $_POST[ $nonce ], $nonce ) && current_user_can( 'switch_themes' ) ) {
					...
					$is_settings_updated_message = __( 'Your setting has been updated.', 'divi-100-login-page' );
					$is_settings_updated_success = true;
				} else {
					$is_settings_updated_message = __( 'Error authenticating request. Please try again.', 'divi-100-login-page' );
				}
			}
					...
					<?php if ( $is_settings_updated ) { ?>
						<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible <?php echo $is_settings_updated_success ? '' : 'error' ?>" style="margin: 0 0 25px 0;">
							...
							<button type="button" class="notice-dismiss">
								<span class="screen-reader-text"><?php _e( 'Dismiss this notice.', 'divi-100-login-page' ); ?></span>
							</button>
						</div>
					<?php } ?>

					<div id="epanel-top">
						<button class="save-button" id="epanel-save-top"><?php _e( 'Save Changes', 'divi-100-login-page' ); ?></button>
					</div>
																	...
																	switch ( $field['type'] ) {
																		...
																		case 'toggle':
																			...
																			echo sprintf(
																				...
																				esc_html__( 'Enable', 'divi-100-login-page' ),
																				esc_html__( 'Disable', 'divi-100-login-page' )
																			);
																		...
																		case 'color':
																			printf(
																				...
																				esc_attr( $field_id ),
																				esc_html__( 'Reset Color', 'divi-100-login-page' )
																			);
